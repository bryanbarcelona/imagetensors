...
class NdImageReader(BaseImageReader):
    ...
    def _find_associated_files(self) -> None:
        ...
        self._associated_files = [
            f for f in parent_dir.glob(pattern)
            if f.suffix.lower() in ('.tif', '.stk') and f.is_file()
        ]
    ...
    def _extract_metadata_from_tif(self) -> None:
        ...
        with tifffile.TiffFile(self._associated_files[0]) as tif:
            ...
            if 282 in tags:  # XResolution
                x_res = float(tags[282].value[0])
            else:
                x_res = 1.0
            
            if 283 in tags:  # YResolution
                y_res = float(tags[283].value[0])
            else:
                y_res = 1.0
    ...
    def _build_file_tree(self) -> None:
        ...
        relative_files = [
            str(f).replace(base_path, '') 
            for f in self._associated_files
        ]
        ...
        # Validate counts
        if (num_w != self._metadata.channels or 
            num_s != self._stage_count or 
            num_t != self._metadata.frames):
            raise ValueError(
        ...
        for stage in range(self._stage_count):
            stage_files = [
                f for f in relative_files 
                if f'_s{stage+1}' in f or num_s == 1
            ]
            if not stage_files:
            ...
            for time in range(self._metadata.frames):
                ...
                time_files = [
                    f for f in stage_files 
                    if re.search(time_pattern, f) or num_t == 1
                ]
                if not time_files:
                ...
                for channel in range(self._metadata.channels):
                    channel_files = [
                        ...
                        if f'_w{channel+1}' in f or num_w == 1
                    ]
        ...
        for stage_idx, stage_tree in enumerate(self._file_tree):
            ...
            for time_tree in stage_tree:
                ...
                for channel_files in time_tree:
                    ...
                    # Handle 2D images
                    stacks = [
                        np.expand_dims(s, axis=0) if s.ndim == 2 else s
                        for s in stacks
                    ]
